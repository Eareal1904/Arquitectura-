...
\label{sec:org22763a4}

\begin{frame}[label={sec:orgcff286b}]{Objetivos del SO (G9, 11, 279)}
\begin{itemize}
...
\begin{frame}[label={sec:org85c7a91}]{Funciones del SO relacionadas con memoria (G9, 11, 279)}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org2040585}]{Estados de un Proceso (Planificación)}
\begin{center}
\includegraphics[width=0.85\textwidth]{images/Procesos.png}
\end{center}
\end{frame}
\section{Planificación y Memoria Virtual (Sección)}
\label{sec:orgdc9e131}

\begin{frame}[label={sec:orgfffcf79}]{Planificación de la memoria (G9, 11, 280)}
\begin{itemize}
...
\begin{frame}[label={sec:orgb5d4f43}]{Principios de Memoria Virtual (G9, 11, 282)}
\begin{itemize}
...
\begin{frame}[label={sec:orgdbb5966}]{Paginación (G9, 11, 283)}
\begin{columns}
...
\label{sec:orgef0cb99}

\begin{frame}[label={sec:org6df63a5}]{Traducción de direcciones virtuales a físicas (G9, 11, 285)}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org1108f7e}]{TLB (Translation Lookaside Buffer) (G9, 11, 287)}
\begin{itemize}
...
\begin{frame}[label={sec:orgf752d87}]{Page Faults y reemplazo de páginas (G9, 11, 290)}
\begin{itemize}
...
\label{sec:org70d00aa}

\begin{frame}[label={sec:orge685cb3}]{¿Qué es una Máquina Virtual? (G9, 11, 292)}
\begin{itemize}
...
\begin{frame}[label={sec:orgf70cce9}]{¿Por qué utilizar Máquinas Virtuales?}
\begin{itemize}
...
\begin{frame}[label={sec:org5a6a4a6}]{Host OS y Guest OS (G9, 11, 292)}
\begin{block}{Host OS}
:PROPERTIES:
:BEAMER\textsubscript{col}: 0.5
...
\begin{frame}[label={sec:orgba8add5}]{Hipervisor: ¿Qué hace?}
\begin{itemize}
...
\begin{frame}[label={sec:orgd26163d}]{Hipervisores: Tipo 1 y Tipo 2 (G9, 11, 295)}
***Texto
:PROPERTIES:
:BEAMER\textsubscript{col}: 0.45
...
\begin{frame}[label={sec:orgc1e50ce}]{Comparación entre Tipo 1 y Tipo 2}
\begin{center}
\begin{tabular}{ll}
Tipo 1 & Tipo 2\\[0pt]
\hline
...
\begin{frame}[label={sec:org78125f1}]{Virtualización de Memoria (G9, 11, 298)}
\begin{itemize}
...
\begin{frame}[label={sec:org5f8e2ef}]{Aceleración por Hardware}
\begin{itemize}
\item La traducción de memoria puede ser costosa.
\item Los procesadores modernos incorporan soporte para
\end{itemize}
virtualización.
\begin{itemize}
\item Intel utiliza Extended Page Tables (EPT).
\item AMD incorpora tecnologías equivalentes mediante AMD-V.
\item Estas tecnologías reducen la sobrecarga y mejoran el
\end{itemize}
rendimiento.
\end{frame}

\begin{frame}[label={sec:orga9209ff}]{WSL y su relación con las Máquinas Virtuales}
\begin{itemize}
\item WSL permite ejecutar Linux dentro de Windows.
\item WSL 1 no utiliza una máquina virtual.
\item WSL 2 ejecuta un kernel real de Linux mediante
\end{itemize}
una VM ligera.
\begin{itemize}
\item Utiliza Hyper-V como tecnología de
virtualización.
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org409c960}]{WSL vs VirtualBox}
\begin{center}
\begin{tabular}{ll}
VirtualBox & WSL 2\\[0pt]
\hline
VM completa & VM ligera\\[0pt]
Configuración manual & Configuración automática\\[0pt]
Mayor consumo de recursos & Menor consumo\\[0pt]
Puede ejecutar distintos SO & Diseñado para Linux\\[0pt]
Uso general & Desarrollo de software\\[0pt]
\end{tabular}
\end{center}
\end{frame}

\section{WSL como caso de estudio (Sección)}
\label{sec:orgc804c49}

\begin{frame}[label={sec:orgaac9b63}]{Windows Subsystem for Linux (WSL) (G9, 11, 301)}
\begin{itemize}
...
\begin{frame}[label={sec:org779ea5d}]{WSL1 vs WSL2: arquitectura (G9, 11, 303)}
\begin{columns}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org0ad34d8},fragile]{Integración con el sistema de archivos Windows (G9, 11, 305)}
 \begin{itemize}
...
\begin{frame}[label={sec:orge5d2d1e}]{WSL y memoria virtual (G9, 11, 307)}
\begin{itemize}
...
\end{frame}
\section{Referencias}
\label{sec:org11e13e1}
\begin{frame}[allowframebreaks]{Bibliografía}
\printbibliography
\end{frame}

\section{Indicaciones}
\label{sec:org8c932e5}
