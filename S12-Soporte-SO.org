...
:BEAMER_col: 0.5
:END:
- La dirección lógica (virtual) consta de un número de página y un desplazamiento[cite: 1, 2].
- La dirección física real consta de un número de marco y un desplazamiento[cite: 1, 2].
- El procesador utiliza la tabla de páginas para producir la dirección física a partir de la lógica[cite: 1, 2].
...
- A diferencia de la paginación, la segmentación divide la memoria en espacios dinámicos y de tamaño variable.
- Este esquema suele ser visible para el programador.
- Simplifica el manejo de estructuras de datos crecientes, facilita la compartición de código y permite asignar privilegios de acceso.
